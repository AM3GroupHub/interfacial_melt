...
Concentration structure factors at 1500 K: 11 compositions at 0 and 10 GPa, using the last 50% of each trajectory. The original PDF is `reference.pdf`.
...
Outputs are the 1500 K PDF/PNG and a refitted summary in `output/`. The script retains all 44 fits at 1500/1800 K to preserve the original seed ordering: `k < 0.8 angstrom^-1`, 500 residual-bootstrap replicates, seed `20260824`. Shading shows pointwise 95% fit confidence bands, not frame-level errors. No NPZ files or trajectories are needed.
