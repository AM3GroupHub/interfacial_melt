...
SIXFOLD_SUMMARY = ROOT / "sixfold_fraction_bar" / "sixfold_fraction_summary.csv"
ENERGY_0GPA = ROOT / "0GPa_liquid" / "normalized_energy_0GPa_liquid.csv"
ENERGY_10GPA = ROOT / "10GPa_liquid" / "normalized_energy_10GPa_liquid.csv"
...
def read_csv_with_x(path, value_field, x_field="x_B"):
    data = {}
    with open(path, newline="", encoding="utf-8") as handle:
        for row in csv.DictReader(handle):
            key = (row["formula"], float(row["temperature_K"]))
            data[key] = (float(row[x_field]), float(row[value_field]))
    return data
...
def build_plot_data():
    ...
    return fraction_data, pot_0gpa, pot_0gpa_err, fit_source
...
def plot_temperature(temp, fraction_data, pot_0gpa, pot_0gpa_err, fit_source):
    ...
    full_x = []
    full_y = []
    for (formula, row_temp), (x_b, value) in fit_source.items():
        if row_temp == temp:
            full_x.append(x_b)
            full_y.append(value)
    ...
    fig.tight_layout()
    out_dir = ROOT / "sixfold_fraction_bar"
    out_dir.mkdir(exist_ok=True)
    out_path = out_dir / f"sixfold_fraction_energy_overlay_{temp:.0f}K.png"
    fig.savefig(out_path, dpi=300, bbox_inches="tight")
    fig.savefig(out_dir / f"sixfold_fraction_energy_overlay_{temp:.0f}K.pdf", bbox_inches="tight")
    plt.close(fig)
    return out_path


def write_summary_csv(fraction_data, pot_0gpa, fit_source):
    out_dir = ROOT / "sixfold_fraction_bar"
    out_dir.mkdir(exist_ok=True)
    out_path = out_dir / "sixfold_fraction_energy_overlay_summary.csv"
    with open(out_path, "w", newline="", encoding="utf-8") as handle:
        ...
        writer.writerow([
            ...
            "composition",
            "pressure",
            "sixfold_fraction_mean",
            "sixfold_fraction_sem",
            "poteng_norm_0gpa",
        ])
            ...
            for composition, _, _ in COMPOSITIONS:
                for pressure in PRESSURES:
                    ...
                    writer.writerow([
                        ...
                        f"{stats['mean']:.8f}",
                        f"{stats['sem']:.8f}",
                        f"{pot_0gpa[(composition, temp)]:.8f}",
                    ])
    return out_path


def main():
    fraction_data, pot_0gpa, pot_0gpa_err, fit_source = build_plot_data()
    summary_csv = write_summary_csv(fraction_data, pot_0gpa, fit_source)
    out_paths = [plot_temperature(temp, fraction_data, pot_0gpa, pot_0gpa_err, fit_source) for temp in TEMPERATURES]
