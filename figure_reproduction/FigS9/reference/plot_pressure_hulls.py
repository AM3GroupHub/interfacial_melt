...
X_LABEL = "B fraction"
Y_LABEL_0K = "Formation energy at 0 K (eV/atom)"
...
def collect_data():
    ...
    for pressure in PRESSURES:
        raw[pressure] = {}
        for phase in PHASES:
            phase_dir = ROOT / f"{pressure}GPa" / phase
...
def write_csv(rows):
    with (ROOT / "hull_summary.csv").open("w", newline="", encoding="utf-8") as f:
...
def plot_one(temp, plot_data, out_png, out_pdf):
    ...
    fig.savefig(ROOT / out_png, dpi=DPI)
    fig.savefig(ROOT / out_pdf)
    plt.close(fig)
...
def plot_combined(plot_data):
    ...
    fig.savefig(ROOT / "hulls_0K_1800K.png", dpi=DPI)
    fig.savefig(ROOT / "hulls_0K_1800K.pdf")
    plt.close(fig)
...
if __name__ == "__main__":
    ...
    write_csv(rows)
    plot_one(0, plot_data, "hulls_0K.png", "hulls_0K.pdf")
    plot_one(1800, plot_data, "hulls_1800K.png", "hulls_1800K.pdf")
    plot_combined(plot_data)
    print(f"WROTE {ROOT / 'hull_summary.csv'}")
    print(f"WROTE {ROOT / 'hulls_0K.png'}")
    print(f"WROTE {ROOT / 'hulls_1800K.png'}")
    print(f"WROTE {ROOT / 'hulls_0K_1800K.png'}")
