...
FINAL_ROOT = ROOT.parent
FEB_SOURCE = FINAL_ROOT / "0GPa_liquid" / "normalized_energy_0GPa_liquid.csv"
CRB_SOURCE = ROOT / "normalized_energy_CrB_0GPa_1500_1800.csv"
...
def main():
    ...
    out_base = ROOT / "combined_FeB_CrB_free_energy_0GPa_1500K_1800K_with_config_entropy"
    ...
    fig.savefig(out_base.with_suffix(".eps"), dpi=600, bbox_inches="tight")
    plt.close(fig)
    print(f"Wrote {out_base.with_suffix('.png')}")
    print(f"Wrote {out_base.with_suffix('.pdf')}")
    print(f"Wrote {out_base.with_suffix('.eps')}")
