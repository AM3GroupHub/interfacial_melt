...
PV_SOURCE = ROOT / "data" / "pv_composition_10gpa_1500k.csv"
ENERGY_SOURCE = ROOT / "data" / "potential_energy_10gpa_1500k.csv"
...
def main():
    ...
    with open(ENERGY_SOURCE, newline="", encoding="utf-8") as handle:
        for row in csv.DictReader(handle):
            if float(row["temperature_K"]) != TEMPERATURE:
                continue
            formula = row["formula"]
            rows.append((float(row["x_B"]), formula))
    ...
    y_text = max(max(pot_values), max(pot_plus_pv)) - 0.03 * (max(max(pot_values), max(pot_plus_pv)) - min(min(pot_values), min(pot_plus_pv)))
    ...
    out_path = OUTPUT_DIR / "pv_potential_overlay_10GPa_1500K_4b_rightonly.png"
    fig.savefig(out_path, dpi=600, bbox_inches="tight")
    fig.savefig(OUTPUT_DIR / "pv_potential_overlay_10GPa_1500K_4b_rightonly.pdf", bbox_inches="tight")
    plt.close(fig)
