...
ENDPOINT_FILES = {
    "Cr": ("Cr250", ROOT / "Cr" / "Cr" / "NPT" / "Cr250_G.txt"),
    "B": ("B144", ROOT / "B" / "B_small" / "NPT" / "B144_G.txt"),
}
ENERGY_COLUMNS = [
...
# Match the geometry of the upper free-energy row in
# final/a_new/free_energy_d2g_0GPa_1500K_1800K_with_config_entropy.pdf.
# Values are in PDF points so the exported page and axes remain deterministic.
REFERENCE_PAGE_WIDTH_PT = 239.4942117939
REFERENCE_AXES_LEFT_PT = 36.2625
REFERENCE_AXES_WIDTH_PT = 89.9507575758
REFERENCE_AXES_GAP_PT = 11.9198232323
REFERENCE_AXES_HEIGHT_PT = 99.3470545455
REFERENCE_BOTTOM_MARGIN_PT = 26.5125
REFERENCE_TOP_MARGIN_PT = 9.48125
REFERENCE_PAGE_HEIGHT_PT = REFERENCE_BOTTOM_MARGIN_PT + REFERENCE_AXES_HEIGHT_PT + REFERENCE_TOP_MARGIN_PT
FIGSIZE = (REFERENCE_PAGE_WIDTH_PT / 72.0, REFERENCE_PAGE_HEIGHT_PT / 72.0)
...
def collect_records():
    ...
    records = []
    for path in sorted(ROOT.glob("**/*_G.txt")):
...
def write_csv(records):
    out_path = ROOT / "normalized_energy_CrB_0GPa_1500_1800.csv"
...
def main():
    ...
    panels = [prepare_panel(records, temperature) for temperature in PLOT_TEMPERATURES]
    ...
    axes_bottom = REFERENCE_BOTTOM_MARGIN_PT / REFERENCE_PAGE_HEIGHT_PT
    axes_width = REFERENCE_AXES_WIDTH_PT / REFERENCE_PAGE_WIDTH_PT
    axes_height = REFERENCE_AXES_HEIGHT_PT / REFERENCE_PAGE_HEIGHT_PT
    left_x = REFERENCE_AXES_LEFT_PT / REFERENCE_PAGE_WIDTH_PT
    right_x = (
        REFERENCE_AXES_LEFT_PT + REFERENCE_AXES_WIDTH_PT + REFERENCE_AXES_GAP_PT
    ) / REFERENCE_PAGE_WIDTH_PT
    left_ax = fig.add_axes([left_x, axes_bottom, axes_width, axes_height])
    right_ax = fig.add_axes([right_x, axes_bottom, axes_width, axes_height], sharey=left_ax)
    axes = np.array([left_ax, right_ax])
    for ax, panel in zip(axes, panels):
        plot_panel(ax, panel, energy_ymin)
        ax.set_ylim(energy_ymin, energy_ymax)
        ax.set_yticks(energy_yticks)
    axes[1].tick_params(labelleft=False)
    axes[0].set_ylabel(r"$H-T(S_{vib}+S_{mix})$ [eV/atom]", fontsize=AXIS_LABEL_FONTSIZE)
    for ax in axes:
        ax.set_xlabel(r"$x$ in Cr$_{1-x}$B$_x$", fontsize=AXIS_LABEL_FONTSIZE)
    png_path = ROOT / "free_energy_0GPa_1500K_1800K_with_config_entropy.png"
    pdf_path = ROOT / "free_energy_0GPa_1500K_1800K_with_config_entropy.pdf"
    eps_path = ROOT / "free_energy_0GPa_1500K_1800K_with_config_entropy.eps"
