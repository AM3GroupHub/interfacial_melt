...
def main():
    ...
    for row, pressure in enumerate((0, 10)):
        source = ROOT.parent / f"{pressure}GPa_liquid" / f"normalized_energy_{pressure}GPa_liquid.csv"
    ...
    for extension in ("png", "pdf"):
        ...
        target = ROOT / f"curvature_fit_5th_order{suffix}.{extension}"
        fig.savefig(target, dpi=600)
        print(target)
    plt.close(fig)
    with (ROOT / "curvature_with_uncertainty.csv").open("w", newline="", encoding="utf-8") as handle:
