...
SOURCE = ROOT / "data" / "csv" / "free_energy_10gpa_1500_1800.csv"
...
def fit_curve(x, y, temperature):
    degree = min(FIT_DEGREES[temperature], len(x) - 1)
...
def draw_custom_legend(ax, color, temperature):
    legend_x0 = 0.08
    legend_x1 = 0.17
    text_x = 0.20
    ...
    ax.text(text_x, y_rows[0], f"10 GPa, {temperature:.0f} K", transform=ax.transAxes, fontsize=6, color=color, va="center")
...
def prepare_panel(rows, temperature):
    ...
    coeff, xs, ys = fit_curve(x, y, temperature)
    return {
...
def plot_panel(ax, panel, baseline):
    ...
    if panel["temperature"] == 1800.0:
        ax.fill_between(xs, ys, baseline, color=LEFT_FILL_COLOR, alpha=0.7, zorder=1)
        ax.plot(xs, ys, "-", color=LEFT_CURVE_COLOR, linewidth=0.8, alpha=0.95, zorder=3)
    else:
        boundary = closest_inflection_point(coeff, 0.6, float(np.min(x)), float(np.max(x)))
        if boundary is None:
            boundary = 0.6
        left_mask = xs <= boundary
        right_mask = xs >= boundary
    ...
    for x_guide in GRAY_GUIDE_X:
        ...
        ax.plot([x_guide, x_guide], [baseline, y_guide], color="gray", linestyle=":", linewidth=0.5, alpha=0.9, zorder=2)
    ...
    ax.axvline(0.8, color="gray", linestyle="--", linewidth=0.6, alpha=0.9)
    ax.text(0.735, 0.125, r"FeB$_4$", fontsize=6, ha="center", va="center")
    draw_custom_legend(ax, color, panel["temperature"])
...
def main():
    ...
    out_path = ROOT / "free_energy_10GPa_1500K_1800K.png"
    fig.savefig(out_path, dpi=600, bbox_inches="tight")
    fig.savefig(ROOT / "free_energy_10GPa_1500K_1800K.pdf", bbox_inches="tight")
    fig.savefig(ROOT / "free_energy_10GPa_1500K_1800K.eps", dpi=600, bbox_inches="tight")
    plt.close(fig)
